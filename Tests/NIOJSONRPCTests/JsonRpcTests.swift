...
extension NIOJSONRPC {
    ...
    final class JSONRPCTests: XCTestCase {
        ...
        func testBadServerResponse1() {
            Framing.Kind.allCases.forEach { framing in
        ...
        func testServerDisconnect() {
            ...
            let eventLoopGroup = MultiThreadedEventLoopGroup(numberOfThreads: System.coreCount)
            // start server
            let server = BadServer(group: eventLoopGroup, framing: .newline)
            _ = try! server.start(host: address.0, port: address.1).wait()
            // connect client
            let client = TCPClient(group: eventLoopGroup)
            _ = try! client.connect(host: address.0, port: address.1).wait()
        ...
        func testDisconnectAfterBadClientRequest() {
            ...
            _ = try! server.start(host: address.0, port: address.1).wait()
            // connect client
            let client = BadClient(group: eventLoopGroup, framing: .newline)
            _ = try! client.connect(host: address.0, port: address.1).wait()
    ...
    private class BadServer {
        ...
        private let framing: Framing.Kind
        private var channel: Channel?

        public init(group: EventLoopGroup, framing: Framing.Kind) {
            self.group = group
        ...
        private class Handler: ChannelInboundHandler {
            ...
            public init(framing: Framing.Kind) {
                self.framing = framing
            }
...
private func encode(_ text: String, _ framing: NIOJSONRPC.Framing.Kind) -> String {
    switch framing {
    case .contentLengthHeader:
        return "Content-Length: \(text.utf8.count)\r\n\r\n" + text
    case .newline:
        return text + "\r\n"
    case .bruteForce:
        return text
...
private func decode(_ buffer: inout ByteBuffer, _ framing: NIOJSONRPC.Framing.Kind) -> Data {
    switch framing {
    case .contentLengthHeader:
        // FIXME
        let all = buffer.readString(length: buffer.readableBytes)!
        let index: Int = all.distance(from: all.startIndex, to: all.range(of: "\r\n\r\n")!.upperBound)
        buffer.moveReaderIndex(to: index-1)
